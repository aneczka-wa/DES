import sys
ip = [x-1 for x in [58, 50, 42, 34, 26, 18, 10, 2,
      60, 52, 44, 36, 28, 20, 12, 4,
      62, 54, 46, 38, 30, 22, 14, 6,
      64, 56, 48, 40, 32, 24, 16, 8,
      57, 49, 41, 33, 25, 17, 9, 1,
      59, 51, 43, 35, 27, 19, 11, 3,
      61, 53, 45, 37, 29, 21, 13, 5,
      63, 55, 47, 39, 31, 23, 15, 7]]

ip_od = [x-1 for x in[40, 8, 48, 16, 56, 24, 64, 32,
        39, 7, 47, 15, 55, 23, 63, 31,
        38, 6, 46, 14, 54, 22, 62, 30,
        37, 5, 45, 13, 53, 21, 61, 29,
        36, 4, 44, 12, 52, 20, 60, 28,
        35, 3, 43, 11, 51, 19, 59, 27,
        34, 2, 42, 10, 50, 18, 58, 26,
        33, 1, 41, 9, 49, 17, 57, 25]]

e = [x-1 for x in [32, 1, 2, 3, 4, 5,
    4, 5, 6, 7, 8, 9,
    8, 9, 10, 11, 12, 13,
    12, 13, 14, 15, 16, 17,
    16, 17, 18, 19, 20, 21,
    20, 21, 22, 23, 24, 25,
    24, 25, 26, 27, 28, 29,
    28, 29, 30, 31, 32, 1]]

S = [
    [14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7,
    0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8,
    4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0,
    15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13],

    [15,1,8,14,6,11,3,4,9,7,2,13,12,0,5,10,
    3,13,4,7,15,2,8,14,12,0,1,10,6,9,11,5,
    0,14,7,11,10,4,13,1,5,8,12,6,9,3,2,15,
    13,8,10,1,3,15,4,2,11,6,7,12,0,5,14,9],

    [10,0,9,14,6,3,15,5,1,13,12,7,11,4,2,8,
    13,7,0,9,3,4,6,10,2,8,5,14,12,11,15,1,
    13,6,4,9,8,15,3,0,11,1,2,12,5,10,14,7,
    1,10,13,0,6,9,8,7,4,15,14,3,11,5,2,12],

    [7,13,14,3,0,6,9,10,1,2,8,5,11,12,4,15,
    13,8,11,5,6,15,0,3,4,7,2,12,1,10,14,9,
    10,6,9,0,12,11,7,13,15,1,3,14,5,2,8,4,
    3,15,0,6,10,1,13,8,9,4,5,11,12,7,2,14],

    [2,12,4,1,7,10,11,6,8,5,3,15,13,0,14,9,
    14,11,2,12,4,7,13,1,5,0,15,10,3,9,8,6,
    4,2,1,11,10,13,7,8,15,9,12,5,6,3,0,14,
    11,8,12,7,1,14,2,13,6,15,0,9,10,4,5,3],

    [12,1,10,15,9,2,6,8,0,13,3,4,14,7,5,11,
    10,15,4,2,7,12,9,5,6,1,13,14,0,11,3,8,
    9,14,15,5,2,8,12,3,7,0,4,10,1,13,11,6,
    4,3,2,12,9,5,15,10,11,14,1,7,6,0,8,13],

    [4,11,2,14,15,0,8,13,3,12,9,7,5,10,6,1,
    13,0,11,7,4,9,1,10,14,3,5,12,2,15,8,6,
    1,4,11,13,12,3,7,14,10,15,6,8,0,5,9,2,
    6,11,13,8,1,4,10,7,9,5,0,15,14,2,3,12],

    [13,2,8,4,6,15,11,1,10,9,3,14,5,0,12,7,
    1,15,13,8,10,3,7,4,12,5,6,11,0,14,9,2,
    7,11,4,1,9,12,14,2,0,6,10,13,15,3,5,8,
    2,1,14,7,4,10,8,13,15,12,9,0,3,5,6,11],
]

Keys = []

P = [x-1 for x in [16,7,20,21,29,12,28,17,
                    1,15,23,26,5,18,31,10,
                    2,8,24,14,32,27,3,9,
                    19,13,30,6,22,11,4,25]]

keyC = [x-1 for x in[57,49,41,33,25,17,9,
    1,58,50,42,34,26,18,
    10,2,59,51,43,35,27,
    19,11,3,60,52,44,36]]

keyD = [x-1 for x in [63,55,47,39,31,23,15,
    7,62,54,46,38,30,22,
    14,6,61,53,45,37,29,
    21,13,5,28,20,12,4]]

Perm2 = [x-1 for x in [14,17,11,24,1,5,
    3,28,15,6,21,10,
    23,19,12,4,26,8,
    16,7,27,20,13,2,
    41,52,31,37,47,55,
    30,40,51,45,33,48,
    44,49,39,56,34,53,
    46,42,50,36,29,32]]

skip = 0
	
def SBox(nr,we):
    wiersz = we[0]*2+we[5]
    kolumna = we[1]*8+we[2]*4+we[3]*2+we[4]
#    print S[nr][wiersz*16+kolumna]
    c = [int(x) for x in bin(S[nr][wiersz*16+kolumna])[2:].zfill(4)]
    return c
    
def Sfuncion(wejscie):
    wyjscie =[]
    for i in range(8):
        wyjscie.extend(SBox(i, wejscie[i*6:i*6+6]))
    return wyjscie

def PPerm(wejscie):
    wyjscie = [0]*32
    for i in range(32):
#        print P[i],len(wejscie),len(wyjscie)
        wyjscie[i] = wejscie[P[i]]
    return wyjscie

def DES(wejscie,tryb):
    TTT=0;
    if(tryb=="D"):
        TTT = 15;
    elif(tryb=="S"):
        TTT = 0;
    #initial perm
    wyjscie = 64*[0]
    for i in range(64):
        wyjscie[i] = wejscie[ip[i]]
    l = wyjscie[:32]
    p = wyjscie[32:]
    
    for i in range(15):
        pom = f(p,Keys[abs(TTT-i)])
#        print "".join([str(x) for x in pom])
        tmp = [l[x]^pom[x] for x in range(32)]
#        print "".join([str(x) for x in tmp])
        l = list(p)
        p = list(tmp)
#        print "".join([str(x) for x in l])
#        print "".join([str(x) for x in p])
    last = 15;
    if(TTT==15):
        last=0;
    pom = f(p,Keys[last])
    tmp = [l[x]^pom[x] for x in range(32)]
    l = list(tmp)
    wyn = list(l)
    wyn.extend(p)
    wyn2 = [0]*64
    for i in range(64):
        wyn2[i] = wyn[ip_od[i]]
    return wyn2


def f(wejscie, klucz):
    wejscie2 = 48*[0];
    for i in range(48):
        wejscie2[i] = wejscie[e[i]]
    wejscie3 = [ wejscie2[i]^klucz[i] for i in range(48)]

    wejscie3 = Sfuncion(wejscie3)
#    print "".join([str(x) for x in wejscie3])

    wejscie3 = PPerm(wejscie3)
    return wejscie3

def przesuniecie(wej, ile):
    b = wej[ile:]
    b.extend(wej[:ile])
    return b

def klucz(wejscie):
    c = [0]*28
    d = [0]*28
    for i in range(28):
        c[i]=wejscie[keyC[i]]
        d[i]=wejscie[keyD[i]]
    for i in range(16):
        if(i==0 or i==1 or i==8 or i==15):
            c = przesuniecie(c,1)
            d = przesuniecie(d,1)
        else:
            c = przesuniecie(c,2)
            d = przesuniecie(d,2)

        wynik = []
        wynik.extend(c)
        wynik.extend(d)
        wynik2=[0]*48
        for i in range(48):
            wynik2[i] = wynik[Perm2[i]]
        Keys.append(wynik2)

def szyfr_file(array,tryb):
    new_file = [];
    #new_file.append(array[0])
    for i in array:
        new_file.append(DES(i,tryb))

    return new_file

def ati(arr):
    return int("".join([str(x) for x in arr]),2)

def write_file(file,file_arr,tryb):
	f = open(file,"wb")
	if(tryb=="S"):
		try:
			f.write(chr(ati(file_arr[0])));
			for i in file_arr[1:]:
				for j in range(8):
					f.write(chr(ati(i[j*8:j*8+8])))
		finally:
			f.close()
	if(tryb=="D"):
		try:
			#f.write(chr(ati(file_arr[0])));
			for i in file_arr[:-1]:
				for j in range(8):
					f.write(chr(ati(i[j*8:j*8+8])))
			
			rem_ost = file_arr[-1];
			for i in range(8-skip):
				f.write(chr(ati(rem_ost[i*8:i*8+8])))
		finally:
			f.close()
	
def read_file(file,tryb):
	file_arr = []
	f = open(file,"rb")
	if(tryb=="S"):
		try:
			byte = f.read(1)
			mini_byte = [];
			i = 1;
			while( byte != ""):
				#file_arr.append(struct.unpack('i',byte)[0])
				mini_byte.extend([int(x) for x in bin(ord(byte))[2:].zfill(8)]);
				if(i==8):
					file_arr.append(mini_byte)
					mini_byte = [];
					i=0;
				#print ord(byte)
				byte = f.read(1)
				i+=1;
		finally:
			f.close()
		if(i!=1):
			dodaj = 9-i;
			mini_byte.extend([0]*8*dodaj)
			file_arr.append(mini_byte)

		mini_byte = [int(x) for x in bin(9-i)[2:].zfill(8)]

		#wkladamy info
		output_text = szyfr_file(file_arr, tryb)
		output_text.insert(0,mini_byte)
	
	elif(tryb=="D"):
		try:
			byte = f.read(1)
			skip = ord(byte);
	#		print skip
			byte = f.read(1);
			mini_byte = [];
			i = 1;
			while( byte != ""):
			#file_arr.append(struct.unpack('i',byte)[0])
				mini_byte.extend([int(x) for x in bin(ord(byte))[2:].zfill(8)]);
				if(i==8):
					file_arr.append(mini_byte)
					mini_byte = [];
					i=0;
				#print ord(byte)
				byte = f.read(1)
				i+=1;
		finally:
			f.close()
		output_text = szyfr_file(file_arr,tryb)
	return output_text

k1 = [int(x) for x in bin(int("AEDDF10235EDD444",16))[2:].zfill(64)]
klucz(k1)
TT=sys.argv[1].upper()
input_text = read_file(sys.argv[2],TT)
write_file(sys.argv[3], input_text,TT)
